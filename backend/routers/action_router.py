...
@router.delete("/logs")
async def clear_action_logs(spreadsheet_id: str = Query(..., description="초기화할 시트 ID")):
    """특정 시트의 히스토리 액션 기록을 완전히 초기화합니다."""
    from services.history_manager import clear_history
    try:
        clear_history(spreadsheet_id)
        return {"message": f"{spreadsheet_id} 시트의 히스토리가 초기화되었습니다."}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"히스토리 초기화 오류: {str(e)}")
